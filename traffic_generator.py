...
async def get_ip():
    try:
        response = await asyncio.get_event_loop().run_in_executor(
            None, 
            lambda: requests.get('https://api.ipify.org', proxies={'https': 'socks5://127.0.0.1:9050'}, timeout=10)
        )
...
async def renew_tor_ip():
    print("Attempting to renew Tor IP...")
    try:
        with Controller.from_port(port=9051) as controller:
            controller.authenticate()
            controller.signal(Signal.NEWNYM)
            await asyncio.sleep(5)  # Wait for the new circuit to be established
...
async def main():
    ...
    total_users = 10  # Reduced for testing
    concurrent_users = 2  # Reduced concurrent users
